...
st.title("Stripe/CharityWater Card Checker")

lista = st.text_input("Enter card (cc|mm|yy|cvv):", "")

if st.button("Check"):
    if not lista or len(lista.split("|")) < 4:
        st.error('Please enter card as cc|mm|yy|cvv')
    else:
        cc, mm, yy, cvv = [x.strip() for x in lista.split('|')[:4]]
        ...
        if "error" in stripe_json:
            st.error(f"Stripe error: {stripe_json['error']}")
        else:
            payment_method_id = stripe_json.get('id')
            if not payment_method_id:
                st.error("No payment method ID returned, aborting.")
            else:
                ...
                st.subheader("Stripe Response")
                st.json(stripe_json)
                st.subheader("CharityWater Response")
                st.json(charity_json)